...
def render_forecast(forecast: Any, historical_timeline: Optional[List[Dict]] = None) -> str:
    ...
    for pt in points:
        ...
        hist_str = f" | hist~avg +{int(hist_rise)} mg/dL" if hist_rise else ""
    ...
    if historical_timeline and [ht for ht in historical_timeline if ht.get("avg_glucose_rise_mgdl")]:
        valid_rises = [m["avg_glucose_rise_mgdl"] for m in historical_timeline if m.get("avg_glucose_rise_mgdl")]
        if valid_rises:
            avg_rise = sum(valid_rises) / len(valid_rises)
            lines.append(f"  {'historical avg rise':7} |~+{int(avg_rise)} mg/dL")
